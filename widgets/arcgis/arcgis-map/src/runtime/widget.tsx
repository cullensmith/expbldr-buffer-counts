...
import {
  ...
  AppStateManager, UrlManager, ViewVisibilityContext, loadArcGISMapComponents, PageVisibilityContext, type FeatureDataRecord, appActions
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<MapWidgetProps, State> {
    ...
    return css`
      ...
      */
      .map-component-container:fullscreen {
        ...
        /*
          ...
          The className of normal Popper is 'popper', the className of fixed panels is 'panel-container' and the className of MobilePanel is 'popper mobile-panel-popper'. The role attribute of both normal Popper and MobilePanel is 'dialog'.
        */
        .map-tool-popper, .map-tool-mobile-panel, .popper[role="dialog"], .popper.jimu-dropdown-menu, .panel-container {
          z-index: 9 !important;
        }
      }
      ...
      */
      ...
      .map-fix-layout {
        ...
        /* widgets in map have is-widget class */
        .is-widget {
      ...
      &.map-is-design-mode.custom-layout-editing {
        .map-tool-layout {
          pointer-events: auto;
        }
        ...
        .draggable-map-tool-shell {
          ...
          * {
          ...
          .placeholder-map-tool .remove-btn {
            pointer-events: auto !important;
          }
        }
      }

      &.not-custom-layout-editing {
        .exbmap-ui.exbmap-ui-tool-shell.exbmap-ui-tool-shell-Placeholder {
      ...
      */
      ...
      .esri-features .esri-feature-content a[href], .esri-features .esri-widget__table a[href] {
  ...
  static mapExtraStateProps = (state: IMState, props: AllWidgetProps<IMConfig>): ExtraMapWidgetProps => {
    ...
    const isLiveMode = checkIsLive(appMode)
    ...
    const widthBreakpoint = state.widgetsState?.[mapWidgetId]?.widthBreakpoint
    const isPcLayoutSize = !!(widthBreakpoint && widthBreakpoint !== 'xsmall')
    const isCustomLayoutEditing = !!(config?.layoutIndex === 'custom-layout' && props.isInlineEditing && isDesignMode && isPcLayoutSize)
  ...
  componentDidUpdate (prevProps: MapWidgetProps, prevState: State) {
    ...
    // try to update inline editing state if config.layoutIndex changed
    this.tryUpdateInlineEditingIfConfigChanged(prevProps)

    // try to update inline editing state if appMode changed
    this.tryUpdateInlineEditingIfAppModeChanged(prevProps)

    // try to refresh custom layout toolbar if isInlineEditing changed
    this.tryRefreshCustomLayoutToolbarIfInlineEditingChanged(prevProps)
  }

  tryUpdateInlineEditingIfConfigChanged (prevProps: MapWidgetProps) {
    const preLayoutIndex = prevProps?.config?.layoutIndex || 0
    const currLayoutIndex = this.props?.config?.layoutIndex || 0

    if (currLayoutIndex !== preLayoutIndex) {
      if (currLayoutIndex === 'custom-layout') {
        // default layout => custom layout
        // enable inline editing for custom layout
        if (this.props.isDesignMode && this.state.widthBreakpoint && this.state.widthBreakpoint !== 'xsmall') {
          // only enable inline editing for custom layout in design mode
          this.updateInlineEditingState(true)
        }
      } else if (preLayoutIndex === 'custom-layout') {
        // custom layout => default layout
        // disable inline editing for default layout
        this.updateInlineEditingState(false)
      }
    }
  }

  tryUpdateInlineEditingIfAppModeChanged (prevProps: MapWidgetProps) {
    const preIsDesignMode = !!(prevProps?.isDesignMode || false)
    const currIsDesignMode = !!(this.props?.isDesignMode || false)

    if (preIsDesignMode && !currIsDesignMode) {
      // design mode -> live mode
      if (this.props.isInlineEditing) {
        // inline editing is only enabled in design mode, disable it in live mode
        this.updateInlineEditingState(false)
      }
    }
  }

  tryRefreshCustomLayoutToolbarIfInlineEditingChanged (prevProps: MapWidgetProps) {
    // both map widget and layout framework can modify isInlineEditing, we need to check it here to make sure custom layout toolbar can be updated correctly
    const preIsInlineEditing = !!(prevProps?.isInlineEditing || false)
    const currIsInlineEditing = !!(this.props?.isInlineEditing || false)

    if (preIsInlineEditing !== currIsInlineEditing) {
      this.asyncRefreshCustomLayoutToolbar()
    }
  }

  updateInlineEditingState (isInlineEditing: boolean) {
    if (this.props.isInlineEditing !== isInlineEditing) {
      const widgetId = this.props.id
      getAppStore().dispatch(appActions.setWidgetIsInlineEditingState(widgetId, isInlineEditing))

      // We don't need to call this.asyncRefreshCustomLayoutToolbar() here because we do it in this.tryRefreshCustomLayoutToolbarIfInlineEditingChanged() method.
      // this.asyncRefreshCustomLayoutToolbar()
    }
  }

  asyncRefreshCustomLayoutToolbar () {
    setTimeout(() => {
      // make sure custom layout toolbar item can be updated correctly
      getAppStore().dispatch(appActions.widgetToolbarStateChange(this.props.id, ['arcgis-map-custom-layout']))
    }, 100)
  }
  ...
  onResize = ({ width, height }) => {
    ...
    const widthBreakpoint = (width <= 545 && width > 0) ? 'xsmall' : 'other'
    const nowTime = Date.now()
    const resizeCount = (this.state.resizeCount || 0) + 1
    ...
    const widthBreakpointInAppState = appState?.widgetsState?.[widgetId]?.widthBreakpoint

    if (widthBreakpoint !== widthBreakpointInAppState) {
      const action = appActions.widgetStatePropChange(this.props.id, 'widthBreakpoint', widthBreakpoint)
  ...
  getInnerContent = (isMapInVisibleArea: boolean) => {
    // onResize is always triggered after map widget mounted, so we need to check resizeCount to make sure lastResizeTime is valid.
    const lastResizeTime = this.state.resizeCount >= 2 ? this.state.lastResizeTime : null
      ...
      if (!(this.props.useDataSources && this.props.useDataSources[0] && this.props.useDataSources[0].dataSourceId)) {
        return (
          <div className='w-100 h-100' ref={ref => { this.parentContainer = ref }}>
            <div
              css={this.getInnerContentStyle()}
              className='w-100 h-100 map-component-container'
              ref={this.container}
            >
  ...
  render () {
    ...
    const className = classNames(`jimu-widget arcgis-map ${this.mapRootClassName}`, {
      'map-is-live-mode': this.props.isLiveMode,
      'map-is-design-mode': this.props.isDesignMode,
      'custom-layout-editing': this.props.isCustomLayoutEditing,
      'not-custom-layout-editing': !this.props.isCustomLayoutEditing
    })
