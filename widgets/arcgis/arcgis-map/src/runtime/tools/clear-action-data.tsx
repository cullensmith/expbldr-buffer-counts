...
export default class ClearActionData extends BaseTool<BaseToolProps, State> {
  ...
  getContent = (mapWidgetId: string) => {
    ...
    const disabled = dropdownItems.length === 0 && this.props.isCustomLayoutEditing

    if (dropdownItems.length > 0 || this.props.isCustomLayoutEditing) {
      return (
        <div className='clear-action-data-map-tool' css={this.getStyle()} title={this.getTitle()}>
          <Dropdown
            direction='down'
            size='sm'
          ...
          >
            <DropdownButton icon arrow={false} size='sm' type='default' disabled={disabled}>
      ...
      return <div className='clear-action-data-map-tool hidden-map-tool'></div>
    }
  }
}
