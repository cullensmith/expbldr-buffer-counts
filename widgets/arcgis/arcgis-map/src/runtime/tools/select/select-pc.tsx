...
import {
  ...
  type FeatureLayerQueryParams, type IntlShape, DataSourceSelectionMode, focusElementInKeyboardMode
} from 'jimu-core'
...
import { HorizontalVertical, type ToolPanelPlacement } from '../../layout/config'
...
interface SketchCreateOptions {
  [tool: string]: __esri.sketchUtilsSetCustomToolsCreateToolOptionsCreateOptions
}
...
export class SelectPCTool extends React.PureComponent<Props, States> {
  ...
  tryUpdateActiveToolInfoWhenPropsChange (prevProps: Props) {
    ...
    if (preActiveToolName !== currentActiveToolName) {
      ...
      if (prevSelectIsActiveTool && !currentSelectIsActiveTool) {
        // select changes to inactive tool because another map tool gets active, e.g. map layers tool gets active
...
${selectFromCurrentSelectionTip}
  ...
  useKey (lastKeyEventInfo: LastKeyEventInfo, tolearnaceMs: number): boolean {
    if (lastKeyEventInfo) {
      if (lastKeyEventInfo.isKeyDown) {
        ...
        // key is up, we use tolearnaceMs to determine the return value
  ...
  render () {
    ...
    const rootClassName = classNames('select-pc-map-tool', { 'horizontal-direction': isHorizontal, 'vertical-direction': !isHorizontal })
    ...
    const selectToolContainerClassName = classNames('d-flex align-items-center justify-content-center select-tool-container', { 'flex-row': isHorizontal, 'flex-column': !isHorizontal })
    let panelPlacement = this.props.panelPlacement

    if (!panelPlacement) {
      panelPlacement = isHorizontal ? 'bottom-start' : 'right-start'
    }

    return (
      <div className={rootClassName} css={this.getCSSStyle()}>
        <div className={selectToolContainerClassName} ref={ref => { this.btnContainer = ref }}>
          {/* left button */}
          {
            !isSelecting &&
            <button
              ...
              className={classNames('esri-widget--button-like  border-0 select-tool-btn d-flex align-items-center justify-content-center', { selected: this.state.isActive })}
          ...
          <button
            ...
            style={ isHorizontal ? { width: '18px', height: '32px'} : { width: '32px', height: '18px' } }
          ...
          >
            <Icon aria-hidden={true} size={8} icon={IconArrowDown} />
          </button>
          ...
          <button
            ...
            ref={ref => { this.selectResultContainer = ref }}
            onClick={this.clearSelect}
            disabled={!this.state.sketchInitialed}
          >
            <Icon size={16} icon={SelectClearIcon} />
          </button>

          <Popper
            className='map-tool-popper'
        ...
        {
          (!this.state.sketchInitialed) &&
          <div className='select-tool-loader' />
        }
      </div>
    )
  }
