...
import {
  ...
  type MessagesJson, type ImmutableObject, MessageType, UrlManager, JSAPILayerTypes, type IMAppConfig
} from 'jimu-core'
...
export async function goToTargetImmediately (view: __esri.MapView | __esri.SceneView, _target: __esri.Geometry): Promise<void> {
  ...
  if (geometries && geometries.length > 0) {
    const target = geometries[0] as NotMeshGeometry
  ...
  if (apiLayerView) {
    ...
    queryFeatures(jimuMapView, layerToQuery, querySQL, [objectIdField]).then(async (featureSet) => {
      if (featureSet && featureSet.features && featureSet.features.length > 0) {
        ...
        const objectIds = featureSet.features.map(feature => {
          ...
          // feature.getObjectId() will return null if feature.layer is not set. Use attribute field instead to avoid regression.
...
export async function processZoomToFeatures (mapBaseView: __esri.MapView | __esri.SceneView, layer: any, features: __esri.Graphic[]): Promise<__esri.Graphic[]> {
  if (mapBaseView && mapBaseView.type === '3d' && layer && layer.queryFeatures && features) {
    return await loadArcGISJSAPIModules([
      ...
      query.returnGeometry = true
      query.outFields = ['*']
      query.objectIds = features.map(feature => {
        const objectId = feature.getObjectId?.()
        ...
        // feature.getObjectId() will return null if feature.layer is not set. Use attribute field instead to avoid regression.
        return feature.attributes[layer.objectIdField]
      })
...
export function checkIsLive (appMode: AppMode): boolean {
  if (window.jimuConfig.isInBuilder) {
    if (appMode === AppMode.Design) {
      ...
      // Both AppMode.Run and AppMode.Express are live.
      return true
    }
  } else {
    return true
