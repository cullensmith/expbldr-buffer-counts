...
export default function ScalebarSetting (props: ScalebarSettingProps) {
  ...
  const onScalebarLineStyleKeyDown = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault()
      onScalebarLineStyleClick()
    }
  }, [onScalebarLineStyleClick])
  ...
  const onScalebarRulerStyleKeyDown = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault()
      onScalebarRulerStyleClick()
    }
  }, [onScalebarRulerStyleClick])
  ...
  const onScalebarNumberStyleKeyDown = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault()
      onScalebarNumberStyleClick()
    }
  }, [onScalebarNumberStyleClick])
  ...
  const scaleBarDescId = React.useId()


  return (
    <div className='scalebar-setting' css={domStyle}>
      <label className='d-none' id={scaleBarDescId}>{autoScalebarOptionsTip}</label>
      <SettingRow level={1} tag='label' label={automaticLabel}>
        <Switch
          aria-describedby={scaleBarDescId}
          checked={isAutomatic}
        ...
        <React.Fragment>
          <SettingRow tag='label' label={styleLabel} flow='wrap'>
            <div className='scale-bar-styles d-flex w-100'>
              <div
                className={classNames(scalebarStyleClassName, 'scale-bar-line-style', { selected: style === 'line' })}
                title={scaleBarLineLabel}
                tabIndex={0}
                aria-label={scaleBarLineLabel}
                onClick={onScalebarLineStyleClick}
                onKeyDown={onScalebarLineStyleKeyDown}
              >
                <img src={require('../assets/scalebar_line.svg')} />
              </div>
              <div
                className={classNames(scalebarStyleClassName, 'scale-bar-ruler-style mt-2', { selected: style === 'ruler' })}
                title={scaleBarRulerLabel}
                tabIndex={0}
                aria-label={scaleBarRulerLabel}
                onClick={onScalebarRulerStyleClick}
                onKeyDown={onScalebarRulerStyleKeyDown}
              >
                <img src={require('../assets/scalebar_ruler.svg')} />
              </div>
              <div
                className={classNames(scalebarStyleClassName, 'scale-bar-number-style mt-2', { selected: style === 'number' })}
                title={scaleBarNumberLabel}
                tabIndex={0}
                aria-label={scaleBarNumberLabel}
                onClick={onScalebarNumberStyleClick}
                onKeyDown={onScalebarNumberStyleKeyDown}
              >
                <span>1:1000</span>
              </div>
            </div>
