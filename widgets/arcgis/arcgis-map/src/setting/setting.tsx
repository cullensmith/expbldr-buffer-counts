...
import { type IMConfig, SceneQualityMode, type CustomLODs, type ScaleRange, type IMScalebarOptions, type IMLocateOptions, type IMOverviewMapOptions } from '../config'
...
import LocateSetting from './components/locate-setting'
import OverviewMapSetting from './components/overview-map-setting'
import {
...
} from '../runtime/builder/custom-layout-utils'
...
type MapSettingProps = AllWidgetSettingProps<IMConfig> & ExtraProps

export default class Setting extends React.PureComponent<MapSettingProps, State> {
  unmount = false
  ...
  getStyle () {
    ...
    return css`
      .widget-setting-map {
        ...
        .custom-layout-image-component-container {
          position: relative;
          .customize-layout-text-container {
            position: absolute;
            left: 0;
            right: 0;
            top: 0;
  ...
  componentDidUpdate (prevProps: MapSettingProps): void {
    this.tryToUpdateSidePopperMapToolWhenPropsChange(prevProps)
  }

  tryToUpdateSidePopperMapToolWhenPropsChange (prevProps: MapSettingProps) {
    const preToolConfig = prevProps?.config?.toolConfig
    const curToolConfig = this.props?.config?.toolConfig

    if (preToolConfig && curToolConfig && curToolConfig !== preToolConfig) {
      // toolConfig changed
      if (this.state.sidePopperMapTool && !curToolConfig[`can${this.state.sidePopperMapTool}`]) {
        // current opened side popper map tool is disabled now, so we need to close the side popper.
        this.setState({
          sidePopperMapTool: ''
        })
      }
  ...
  onDataSourceSelected = (newSelectedUseDataSources: UseDataSource[], propsUseDataSources: Immutable.ImmutableArray<UseDataSource>) => {
    ...
    const finalUseDataSources = Immutable(newUseDataSources)
    this.integratedDataSourceSetting = {
      id: this.props.id,
      useDataSources: finalUseDataSources,
    }
    ...
    let newIMConfig: IMConfig = updateConfigToMakeSureCustomLayoutValid(this.props.config, finalUseDataSources) || this.props.config
    newIMConfig = newIMConfig
  ...
  onDataSourceRemoved = (currentRemovedDs: UseDataSource): void => {
    ...
    const finalUseDataSources = Immutable(tempUseDataSources)
    const settingChange = {
      id: this.props.id,
      useDataSources: finalUseDataSources,
    } as any
    ...
    let newConfig: IMConfig = updateConfigToMakeSureCustomLayoutValid(this.props.config, finalUseDataSources) || this.props.config
    newConfig = newConfig
  ...
  onMapToolSettingBtnClick = (mapTool: string, canToolName: string, isSwitchChecked: boolean): void => {
    ...
    if (this.mapToolsContainerClickedTimestamp > 0) {
      ...
      if (deltaTime <= 50) {
        // by test, deltaTime is <= 4ms, we use 50ms here
        // don't use 200ms here, otherwise it will result in #30932
  ...
  handleMapToolSwitchChanged = (mapTool: string, canToolName: string, checked: boolean): void => {
    ...
    const affectedMapTools: string[] = [mapTool]
    let newConfig = this.props.config.setIn(['toolConfig', canToolName], checked)

    if (mapTool === 'Select') {
      affectedMapTools.push('SelectState')
    ...
    newConfig = updateConfigForCustomLayoutPlacementWhenSwitchMapTools(newConfig, this.props.useDataSources, affectedMapTools, checked)
  ...
  changeToolLayout = (newLayoutIndex: number | 'custom-layout') => {
    const oldConfig = this.props.config
    const oldLayoutIndex = oldConfig.layoutIndex || 0

    if (newLayoutIndex === oldLayoutIndex) {
      return
    }

    let newConfig = oldConfig

    if (newLayoutIndex === 'custom-layout') {
      // use custom layout
      newConfig = updateConfigForCustomLayoutPlacementWhenSwitchToCustomLayout(newConfig, this.props.useDataSources)
    } else {
      newConfig = newConfig.without('customLayoutPlacement')
    }

    newConfig = newConfig.set('layoutIndex', newLayoutIndex).set('preLayoutIndex', oldLayoutIndex)
  ...
  handleLayoutThumbKeyDown = (e: React.KeyboardEvent<any>, index: number | 'custom-layout') => {
    if (e.key === 'Enter' || e.key === ' ') {
  ...
  render () {
    ...
    const additionalInformationLabel = this.props.intl.formatMessage({ id: 'additionalInformation', defaultMessage: jimuUIDefaultMessages.additionalInformation })
    ...
    const lodSingleMapTip = this.props.intl.formatMessage({ id: 'lodSingleMapTip', defaultMessage: defaultMessages.lodSingleMapTip })
                ...
                <SettingRow>
                  ...
                  >
                    <Button
                      ...
                      disableRipple={true}
                      aria-label={`${additionalInformationLabel} ${lodSingleMapTip}`}
                      tabIndex={isCustomLodRadioChecked ? 0 : -1}
                    >
                      <InfoOutlined />
                    </Button>
                  </Tooltip>
        ...
        <SettingSection>
          ...
          >
            <div className='w-100' ref={this.onMapToolsContainerRef} onClick={this.onMapToolsContainerClick}>
              {Object.keys(ToolModulesConfig).map((key, index) => {
                const toolModuleInfo = ToolModulesConfig[key]
          ...
          >
            ...
            <SettingRow>
              <div className='w-100 d-flex flex-column'>
                <div className='w-100 d-flex justify-content-between'>
                  <div
                    className={classNames('layout-thumbnail-container border d-flex justify-content-center align-items-center', {
                      'selected-item': !this.props.config.layoutIndex
                    })}
                    aria-labelledby='largeAndMediumLayout'
                    tabIndex={0}
                    role='button'
                    onClick={() => { this.changeToolLayout(0) }}
                    onKeyDown={e => { this.handleLayoutThumbKeyDown(e, 0) }}
                  >
                    <Image className='image-component' src={require('./assets/pc-layout-0.svg')} />
                  </div>
                  <div
                    className={classNames('layout-thumbnail-container border d-flex justify-content-center align-items-center', {
                    ...
                    tabIndex={0}
                    role='button'
                    onClick={() => { this.changeToolLayout(1) }}
                    onKeyDown={e => { this.handleLayoutThumbKeyDown(e, 1) }}
                  >
                    <Image className='image-component' src={require('./assets/pc-layout-1.svg')} />
                  </div>
                </div>
                {
                  !isExpressMode() && (
                    <div className='w-100 d-flex justify-content-between mt-3'>
                      <div
                        className={classNames('layout-thumbnail-container border d-flex justify-content-center align-items-center', {
                          'selected-item': this.props.config.layoutIndex === 'custom-layout'
                        })}
                        aria-labelledby='largeAndMediumLayout'
                        tabIndex={0}
                        role='button'
                        onClick={() => { this.changeToolLayout('custom-layout') }}
                        onKeyDown={e => { this.handleLayoutThumbKeyDown(e, 'custom-layout') }}
                      >
                        <div className='custom-layout-image-component-container'>
                          <Image className='image-component' src={require('./assets/custom-layout.svg')}></Image>
                          <div className='d-flex justify-content-center align-items-center customize-layout-text-container'>
                            <span className='customize-layout-text'>{this.props.intl.formatMessage({ id: 'mapAction_Customize', defaultMessage: defaultMessages.mapAction_Customize })}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          >
...

            {
              ...
              <React.Fragment>
                ...
                <label id='enable-client-query-label' className='d-none'>
                  {this.props.intl.formatMessage({ id: 'enableClientSideQuery', defaultMessage: defaultMessages.enableClientSideQuery })}
                </label>
                    ...
                    return (
                      ...
                      >
                        ...
                        <Switch
                          ...
                          aria-describedby='enable-client-query-label'
  ...
  renderMapToolSidePopper () {
    ...
    const scalebarOptions = toolOptions?.ScaleBar
    const locateOptions = toolOptions?.Locate
      ...
      >
        {
          ...
          <React.Fragment>
            ...
            {
              sidePopperMapTool === 'Locate' &&
              <LocateSetting locateOptions={locateOptions} onChange={this.onLocateSettingChange} />
            }
            {
              sidePopperMapTool === 'OverviewMap' &&
              <OverviewMapSetting mapConfig={config} onChange={this.onOverviewMapSettingChange} />
            }
          </React.Fragment>
        }
      </SidePopper>
  ...
  onLocateSettingChange = (locateOptions: IMLocateOptions): void => {
    const newConfig = this.props.config.setIn(['toolOptions', 'Locate'], locateOptions)
    this.updateConfig(newConfig)
  }

  onOverviewMapSettingChange = (overviewMapOptions: IMOverviewMapOptions): void => {
    const newConfig = this.props.config.setIn(['toolOptions', 'OverviewMap'], overviewMapOptions)
    this.updateConfig(newConfig)
  }
}
