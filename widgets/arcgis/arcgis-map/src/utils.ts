...
import type { IMConfig, ScaleRange, OverviewMapPosition } from './config'
...
export function getOverviewMapPosition (config: IMConfig): OverviewMapPosition {
  const layoutIndex = config?.layoutIndex || 0
  const overviewMapOptions = config?.toolOptions?.OverviewMap
  let position = overviewMapOptions?.position

  if (!position) {
    // user doesn't set position in config, calculate the position by layout
    if (layoutIndex === 1) {
      // pc layout2, right top
      position = 'top-right'
    } else {
      // both pc layout1 and custom layout use bottom-right position
