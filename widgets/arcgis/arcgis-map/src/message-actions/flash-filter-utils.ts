import {
  type FieldSchema, type ImmutableObject, dataSourceUtils, MessageActionConnectionType, type DataSource, messageActionUtils, type JimuFieldType, type Message,
  ...
  type SceneLayerDataSource, type BuildingComponentSubLayerDataSource, DataSourceManager, type UseDataSource, type SqlExpression
} from 'jimu-core'
...
export function getFlashFilterActionValueForUseAnyTriggerData (records: DataRecord[]): FlashFilterActionValue {
  ...
  if (records.length > 0) {
    ...
    if (recordMainDs) {
      ...
      // create SQL by record ids, getSQLForAutoBoundDs() handles both normal id case and uniqueId fields case
      const sqlExpression = messageActionUtils.getSQLForAutoBoundDs(recordMainDs, records, useCaseSensitive)

      if (sqlExpression) {
        const imSqlExpression = Immutable(sqlExpression)
        const sqlResult = dataSourceUtils.getArcGISSQL(imSqlExpression, recordMainDs)
...
function isAutoBound (actionConfig: FlashFilterMessageActionConfig): boolean {
  if (actionConfig) {
    const { messageUseDataSource, actionUseDataSource } = actionConfig

    if (messageUseDataSource && actionUseDataSource) {
      return messageUseDataSource.mainDataSourceId && messageUseDataSource.rootDataSourceId &&
      messageUseDataSource.mainDataSourceId === actionUseDataSource.mainDataSourceId &&
      messageUseDataSource.rootDataSourceId === actionUseDataSource.rootDataSourceId
    }
  }
...
async function getWhereSqlByTriggerActionConnection (message: Message, actionConfig: FlashFilterMessageActionConfig, messageDataSource: DataSource, actionDataSource: DataSource): Promise<string> {
  ...
  if (actionConfig.enabledDataRelationShip) {
    ...
    const isAutoBoundCase = isAutoBound(actionConfig)
    ...
    if (messageField && actionField) {
      ...
      const records = tempMessage.records || []
      ...
      if (actionConfig.connectionType === MessageActionConnectionType.UseLayersRelationship) {
        ...
        if (uniqueFormattedMessageFieldValues.length === 0) {
          ...
          // queryRelatedRecords currently does not supports uniqueIds, so we keep getIdField for now
          actionField = actionDataSource.getSchema().fields[actionDataSource.getIdField()]
        }
      } else {
        // "Set custom connection fields" radio checked
        if (!isAutoBoundCase) {
          const originalFieldValues = records.map(record => {
      ...
      if (isAutoBoundCase) {
        // auto bound case
        // create SQL by record ids, getSQLForAutoBoundDs() handles both normal id case and uniqueId fields case
        sqlExpression = messageActionUtils.getSQLForAutoBoundDs(messageDataSource, records, useCaseSensitive)
      } else if (uniqueFormattedMessageFieldValues?.length > 0) {
        // not auto bound case
        sqlExpression = messageActionUtils.getSqlExpressionWidthMessageFieldValues(uniqueFormattedMessageFieldValues, actionField, actionDataSource, useCaseSensitive)
      }

      if (sqlExpression) {
        const imSqlExpression = Immutable(sqlExpression)
        const sqlResult = dataSourceUtils.getArcGISSQL(imSqlExpression, actionDataSource)
